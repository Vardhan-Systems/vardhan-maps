 ...
 * boundaries overlaid. `leaflet` is an optional peer dependency and is imported
 * lazily, so this is SSR-safe. Remember to import "leaflet/dist/leaflet.css".
 */
export function IndiaLeafletMap(props: IndiaLeafletMapProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (tiles !== false) {
        L.tileLayer(tileUrl ?? OSM_URL, {
          attribution: props.tileAttribution ?? OSM_ATTR,
          maxZoom: 19,
        }).addTo(map);
      }

      const districtFeatures = stateName
        ? allDistricts.features.filter(
            (f) => f.properties.state.toLowerCase() === stateName.toLowerCase(),
          )
      ...
      if (level !== "state") {
        layers.push(
          L.geoJSON({ type: "FeatureCollection", features: districtFeatures } as unknown as GeoJsonObject, {
